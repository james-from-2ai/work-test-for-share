/**
 * FIXTURE, not the live test. This is the 2AI Program Manager performance task exactly as it was
 * compiled before this repo became the Evidence Action EVP deployment, kept so that engine.test
 * and airtable.test have a stable, fully known test to drive: six questions in two parts, one
 * 90-minute clock, forward-only, no branching. Those suites check guarantees that hold whatever
 * test is live, so they must not depend on whichever one happens to be compiled into
 * functions/_lib/wt-questions.mjs. Change this file only if a test needs a different fixture,
 * never to keep it in step with the live test.
 */
